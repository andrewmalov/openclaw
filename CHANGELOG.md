...
- Gateway/webchat: resolve orchestrator webchat as the message tool channel when no external channels are configured, prefer webchat over ambiguous multi-channel selection for webchat-originated tool context, and run inline relay when the resolved channel is webchat.
...
- **BREAKING:** ACP dispatch now defaults to enabled unless explicitly disabled (`acp.dispatch.enabled=false`). If you need to pause ACP turn routing while keeping `/acp` controls, set `acp.dispatch.enabled=false`. Docs: https://github.com/andrewmalov/openclaw
...
- Providers: Ollama discovery + docs; Venice guide upgrades + cross-links. (#1606) Thanks @abhaymundhara. https://github.com/andrewmalov/openclaw https://github.com/andrewmalov/openclaw
- Channels: LINE plugin (Messaging API) with rich replies + quick replies. (#1630) Thanks @plum-dawg.
- TTS: Edge fallback (keyless) + `/tts` auto modes. (#1668, #1667) Thanks @steipete, @sebslight. https://github.com/andrewmalov/openclaw
- Exec approvals: approve in-chat via `/approve` across all channels (including plugins). (#1621) Thanks @czekaj. https://github.com/andrewmalov/openclaw https://github.com/andrewmalov/openclaw
- Telegram: DM topics as separate sessions + outbound link preview toggle. (#1597, #1700) Thanks @rohannagpal, @zerone0x. https://github.com/andrewmalov/openclaw

### Changes

- Channels: add LINE plugin (Messaging API) with rich replies, quick replies, and plugin HTTP registry. (#1630) Thanks @plum-dawg.
- TTS: add Edge TTS provider fallback, defaulting to keyless Edge with MP3 retry on format failures. (#1668) Thanks @steipete. https://github.com/andrewmalov/openclaw
- TTS: add auto mode enum (off/always/inbound/tagged) with per-session `/tts` override. (#1667) Thanks @sebslight. https://github.com/andrewmalov/openclaw
- Telegram: treat DM topics as separate sessions and keep DM history limits stable with thread suffixes. (#1597) Thanks @rohannagpal.
- Telegram: add `channels.telegram.linkPreview` to toggle outbound link previews. (#1700) Thanks @zerone0x. https://github.com/andrewmalov/openclaw
- Web search: add Brave freshness filter parameter for time-scoped results. (#1688) Thanks @JonUleis. https://github.com/andrewmalov/openclaw
- UI: refresh Control UI dashboard design system (colors, icons, typography). (#1745, #1786) Thanks @EnzeD, @mousberg.
- Exec approvals: forward approval prompts to chat with `/approve` for all channels (including plugins). (#1621) Thanks @czekaj. https://github.com/andrewmalov/openclaw https://github.com/andrewmalov/openclaw
- Gateway: expose config.patch in the gateway tool with safe partial updates + restart sentinel. (#1653) Thanks @steipete.
- Diagnostics: add diagnostic flags for targeted debug logs (config + env override). https://github.com/andrewmalov/openclaw
- Docs: expand FAQ (migration, scheduling, concurrency, model recommendations, OpenAI subscription auth, Pi sizing, hackable install, docs SSL workaround).
- Docs: add verbose installer troubleshooting guidance.
- Docs: add macOS VM guide with local/hosted options + VPS/nodes guidance. (#1693) Thanks @f-trycua.
- Docs: add Bedrock EC2 instance role setup + IAM steps. (#1625) Thanks @sergical. https://github.com/andrewmalov/openclaw
...
- BlueBubbles: route phone-number targets to DMs, avoid leaking routing IDs, and auto-create missing DMs (Private API required). (#1751) Thanks @tyler6204. https://github.com/andrewmalov/openclaw
...
- Signal: add configurable signal-cli startup timeout + external daemon mode docs. (#1677) https://github.com/andrewmalov/openclaw
...
- TTS: move Telegram TTS into core + enable model-driven TTS tags by default for expressive audio replies. (#1559) Thanks @Glucksberg. https://github.com/andrewmalov/openclaw
- Gateway: add `/tools/invoke` HTTP endpoint for direct tool calls (auth + tool policy enforced). (#1575) Thanks @vignesh07. https://github.com/andrewmalov/openclaw
- Heartbeat: per-channel visibility controls (OK/alerts/indicator). (#1452) Thanks @dlauer. https://github.com/andrewmalov/openclaw
- Deploy: add Fly.io deployment support + guide. (#1570) https://github.com/andrewmalov/openclaw
- Channels: add Tlon/Urbit channel plugin (DMs, group mentions, thread replies). (#1544) Thanks @wca4a. https://github.com/andrewmalov/openclaw

### Changes

- Channels: allow per-group tool allow/deny policies across built-in + plugin channels. (#1546) Thanks @adam91holt. https://github.com/andrewmalov/openclaw
- Agents: add Bedrock auto-discovery defaults + config overrides. (#1553) Thanks @fal3. https://github.com/andrewmalov/openclaw
- CLI: add `openclaw system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://github.com/andrewmalov/openclaw
- CLI: add live auth probes to `openclaw models status` for per-profile verification. (commit 40181afde) https://github.com/andrewmalov/openclaw
...
- Plugins: add optional `llm-task` JSON-only tool for workflows. (#1498) Thanks @vignesh07. https://github.com/andrewmalov/openclaw
...
- Docs: add cron vs heartbeat decision guide (with Lobster workflow notes). (#1533) Thanks @JustYannicc. https://github.com/andrewmalov/openclaw
- Docs: clarify HEARTBEAT.md empty file skips heartbeats, missing file still runs. (#1535) Thanks @JustYannicc. https://github.com/andrewmalov/openclaw
...
- Control UI: ignore bootstrap identity placeholder text for avatar values and fall back to the default avatar. https://github.com/andrewmalov/openclaw https://github.com/andrewmalov/openclaw
...
- Highlight: Lobster optional plugin tool for typed workflows + approval gates. https://github.com/andrewmalov/openclaw
- Lobster: allow workflow file args via `argsJson` in the plugin tool. https://github.com/andrewmalov/openclaw
...
- CLI: add `openclaw update wizard` for interactive channel selection and restart prompts. https://github.com/andrewmalov/openclaw
- Signal: add typing indicators and DM read receipts via signal-cli.
- MSTeams: add file uploads, adaptive cards, and attachment handling improvements. (#1410) Thanks @Evizero.
- Onboarding: remove the run setup-token auth option (paste setup-token or reuse CLI creds instead).
- Docs: add troubleshooting entry for gateway.mode blocking gateway start. https://github.com/andrewmalov/openclaw
- Docs: add /model allowlist troubleshooting note. (#1405)
- Docs: add per-message Gmail search example for gog. (#1220) Thanks @mbelinky.

### Breaking

- **BREAKING:** Control UI now rejects insecure HTTP without device identity by default. Use HTTPS (Tailscale Serve) or set `gateway.controlUi.allowInsecureAuth: true` to allow token-only auth. https://github.com/andrewmalov/openclaw
...
- Control UI: add copy-as-markdown with error feedback. (#1345) https://github.com/andrewmalov/openclaw
- Control UI: drop the legacy list view. (#1345) https://github.com/andrewmalov/openclaw
- TUI: add syntax highlighting for code blocks. (#1200) https://github.com/andrewmalov/openclaw
- TUI: session picker shows derived titles, fuzzy search, relative times, and last message preview. (#1271) https://github.com/andrewmalov/openclaw
- TUI: add a searchable model picker for quicker model selection. (#1198) https://github.com/andrewmalov/openclaw
- TUI: add input history (up/down) for submitted messages. (#1348) https://github.com/andrewmalov/openclaw
- ACP: add `openclaw acp` for IDE integrations. https://github.com/andrewmalov/openclaw
- ACP: add `openclaw acp client` interactive harness for debugging. https://github.com/andrewmalov/openclaw
- Skills: add download installs with OS-filtered options. https://github.com/andrewmalov/openclaw
- Skills: add the local sherpa-onnx-tts skill. https://github.com/andrewmalov/openclaw
- Memory: add hybrid BM25 + vector search (FTS5) with weighted merging and fallback. https://github.com/andrewmalov/openclaw
- Memory: add SQLite embedding cache to speed up reindexing and frequent updates. https://github.com/andrewmalov/openclaw
- Memory: add OpenAI batch indexing for embeddings when configured. https://github.com/andrewmalov/openclaw
- Memory: enable OpenAI batch indexing by default for OpenAI embeddings. https://github.com/andrewmalov/openclaw
- Memory: allow parallel OpenAI batch indexing jobs (default concurrency: 2). https://github.com/andrewmalov/openclaw
- Memory: render progress immediately, color batch statuses in verbose logs, and poll OpenAI batch status every 2s by default. https://github.com/andrewmalov/openclaw
- Memory: add `--verbose` logging for memory status + batch indexing details. https://github.com/andrewmalov/openclaw
- Memory: add native Gemini embeddings provider for memory search. (#1151) https://github.com/andrewmalov/openclaw
- Browser: allow config defaults for efficient snapshots in the tool/CLI. (#1336) https://github.com/andrewmalov/openclaw
- Nostr: add the Nostr channel plugin with profile management + onboarding defaults. (#1323) https://github.com/andrewmalov/openclaw
- Matrix: migrate to matrix-bot-sdk with E2EE support, location handling, and group allowlist upgrades. (#1298) https://github.com/andrewmalov/openclaw
- Slack: add HTTP webhook mode via Bolt HTTP receiver. (#1143) https://github.com/andrewmalov/openclaw
- Telegram: enrich forwarded-message context with normalized origin details + legacy fallback. (#1090) https://github.com/andrewmalov/openclaw
- Discord: fall back to `/skill` when native command limits are exceeded. (#1287)
- Discord: expose `/skill` globally. (#1287)
- Zalouser: add channel dock metadata, config schema, setup wiring, probe, and status issues. (#1219) https://github.com/andrewmalov/openclaw
- Plugins: require manifest-embedded config schemas with preflight validation warnings. (#1272) https://github.com/andrewmalov/openclaw
- Plugins: move channel catalog metadata into plugin manifests. (#1290) https://github.com/andrewmalov/openclaw
- Plugins: align Nextcloud Talk policy helpers with core patterns. (#1290) https://github.com/andrewmalov/openclaw
- Plugins/UI: let channel plugin metadata drive UI labels/icons and cron channel options. (#1306) https://github.com/andrewmalov/openclaw
- Agents/UI: add agent avatar support in identity config, IDENTITY.md, and the Control UI. (#1329) https://github.com/andrewmalov/openclaw
- Plugins: add plugin slots with a dedicated memory slot selector. https://github.com/andrewmalov/openclaw
- Plugins: ship the bundled BlueBubbles channel plugin (disabled by default). https://github.com/andrewmalov/openclaw
- Plugins: migrate bundled messaging extensions to the plugin SDK and resolve plugin-sdk imports in the loader.
- Plugins: migrate the Zalo plugin to the shared plugin SDK runtime. https://github.com/andrewmalov/openclaw
- Plugins: migrate the Zalo Personal plugin to the shared plugin SDK runtime. https://github.com/andrewmalov/openclaw
- Plugins: allow optional agent tools with explicit allowlists and add the plugin tool authoring guide. https://github.com/andrewmalov/openclaw
...
- Usage: add `/usage cost` summaries and macOS menu cost charts. https://github.com/andrewmalov/openclaw
- Security: warn when <=300B models run without sandboxing while web tools are enabled. https://github.com/andrewmalov/openclaw
- Exec: add host/security/ask routing for gateway + node exec. https://github.com/andrewmalov/openclaw
- Exec: add `/exec` directive for per-session exec defaults (host/security/ask/node). https://github.com/andrewmalov/openclaw
- Exec approvals: migrate approvals to `~/.openclaw/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://github.com/andrewmalov/openclaw
- Nodes: add headless node host (`openclaw node start`) for `system.run`/`system.which`. https://github.com/andrewmalov/openclaw
- Nodes: add node daemon service install/status/start/stop/restart. https://github.com/andrewmalov/openclaw
- Bridge: add `skills.bins` RPC to support node host auto-allow skill bins.
- Sessions: add daily reset policy with per-type overrides and idle windows (default 4am local), preserving legacy idle-only configs. (#1146) https://github.com/andrewmalov/openclaw
- Sessions: allow `sessions_spawn` to override thinking level for sub-agent runs. https://github.com/andrewmalov/openclaw
- Channels: unify thread/topic allowlist matching + command/mention gating helpers across core providers. https://github.com/andrewmalov/openclaw
- Models: add Qwen Portal OAuth provider support. (#1120) https://github.com/andrewmalov/openclaw
- Onboarding: add allowlist prompts and username-to-id resolution across core and extension channels. https://github.com/andrewmalov/openclaw
- Docs: clarify allowlist input types and onboarding behavior for messaging channels. https://github.com/andrewmalov/openclaw
- Docs: refresh Android node discovery docs for the Gateway WS service type. https://github.com/andrewmalov/openclaw
- Docs: surface Amazon Bedrock in provider lists and clarify Bedrock auth env vars. (#1289) https://github.com/andrewmalov/openclaw
- Docs: clarify WhatsApp voice notes. https://github.com/andrewmalov/openclaw
- Docs: clarify Windows WSL portproxy LAN access notes. https://github.com/andrewmalov/openclaw
- Docs: refresh bird skill install metadata and usage notes. (#1302) https://github.com/andrewmalov/openclaw
...
- Hooks: add hooks system with bundled hooks, CLI tooling, and docs. (#1028) — thanks @ThomsenDrake. https://github.com/andrewmalov/openclaw
- Media: add inbound media understanding (image/audio/video) with provider + CLI fallbacks. https://github.com/andrewmalov/openclaw
- Plugins: add Zalo Personal plugin (`@openclaw/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://github.com/andrewmalov/openclaw
- Models: add Vercel AI Gateway auth choice + onboarding updates. (#1016) — thanks @timolins. https://github.com/andrewmalov/openclaw
- Sessions: add `session.identityLinks` for cross-platform DM session li nking. (#1033) — thanks @thewilloftheshadow. https://github.com/andrewmalov/openclaw
- Web search: add `country`/`language` parameters (schema + Brave API) and docs. (#1046) — thanks @YuriNachos. https://github.com/andrewmalov/openclaw
...
- **BREAKING:** `openclaw hooks` is now `openclaw webhooks`; hooks live under `openclaw hooks`. https://github.com/andrewmalov/openclaw
...
- Tools: add `exec` PTY support for interactive sessions. https://github.com/andrewmalov/openclaw
...
- Security: default-deny slash/control commands unless a channel computed `CommandAuthorized` (fixes accidental “open” behavior), and ensure WhatsApp + Zalo plugin channels gate inline `/…` tokens correctly. https://github.com/andrewmalov/openclaw
